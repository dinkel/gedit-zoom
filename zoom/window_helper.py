...
from gtk import ActionGroup, Action, accelerator_parse
...
class WindowHelper:
    ...
    _UI_FILE = join(dirname(__file__), "zoom_menu.ui.xml")
    ...
    def __init__(self, window):
        ...
        map(self._initialize_viewhelper, self._window.get_views())

        self._tab_add_handler = self._window.connect(
    ...
    def deactivate(self):
        ...
        map(self._deactivate_viewhelper, self._window.get_views())
        self._views = None
        self._remove_menu()
    ...
    def _insert_menu(self):
        ...
        self._action_group = ActionGroup("ZoomPluginActions")
        action_submenu = Action("ZoomSubMenu", _("_Zoom"), _("_Zoom"), None)
        action_enlarge_font = Action(
            "EnlargeFont", _("Zoom _In"), _("Zoom _In"), None)
        action_enlarge_font.connect('activate', self._enlarge_font)
        action_shrink_font = Action(
            "ShrinkFont", _("Zoom _Out"), _("Zoom _Out"), None)
        action_shrink_font.connect("activate", self._shrink_font)
        action_reset_font = Action(
            "OriginalSize", _("_Reset"), _("_Reset"), None)
        ...
        self._action_group.add_action_with_accel(
            action_enlarge_font, self._ENLARGE_ACCELERATORS[0])
        self._action_group.add_action_with_accel(
            action_shrink_font, self._SHRINK_ACCELERATORS[0])
        self._action_group.add_action_with_accel(
            action_reset_font, self._RESET_ACCELERATORS[0])
        manager.insert_action_group(self._action_group)
        self._ui_id = manager.add_ui_from_file(self._UI_FILE)
    ...
    def _setup_supplementary_accelerators(self):
        ...
        self._connect_accelerators(
            self._ENLARGE_ACCELERATORS[1:], self._enlarge_font)
        self._connect_accelerators(
            self._SHRINK_ACCELERATORS[1:], self._shrink_font)
        self._connect_accelerators(
            self._RESET_ACCELERATORS[1:], self._reset_font)
    ...
    def _enlarge_font(self, *args):
        ...
        if self._has_active_view(): self._get_active_viewhelper().enlarge_font()

    def _shrink_font(self, *args):
        """ Callback to shrink the font on menu click or main accelerator. """
        if self._has_active_view(): self._get_active_viewhelper().shrink_font()

    def _reset_font(self, *args):
        """ Callback to reset the font on menu click or main accelerator. """
        if self._has_active_view(): self._get_active_viewhelper().reset_font()
