...
from gtk.gdk import CONTROL_MASK, SCROLL_UP, SCROLL_DOWN
...
class ViewHelper:
    ...
    def __init__(self, view):
        """ Constructor. """
        self._view = view
        self._scroll_handler = self._view.connect(
            "scroll_event", self._on_scrolling)
    ...
    def _on_scrolling(self, view, event):
        ...
        if (event.state & CONTROL_MASK):
            if event.direction == SCROLL_UP:
                self.enlarge_font()
                return True
            elif event.direction == SCROLL_DOWN:
                self.shrink_font()
                return True
        return False
