...
dummy:
	echo "Possible targets are: dist, tgz, zip, update-locales, create-locale LOCALE={ll[_CC]}, test, disttest, clean, mrproper\n"
...
create-locale: _generate-pot _create_localedir
	if [ -n "$(LOCALE)" ] ; then \
		if [ ! -e "zoom/locale/$(LOCALE)/LC_MESSAGES/$(DOMAIN).po" ] ; then \
			msginit --locale=$(LOCALE) \
				    --input=$(DOMAIN).pot \
				    --output="zoom/locale/$(LOCALE)/LC_MESSAGES/$(DOMAIN).po" ; \
		    sed -i 's/PACKAGE/$(APPLICATION)/g' zoom/locale/$(LOCALE)/LC_MESSAGES/$(DOMAIN).po ; \
			echo "Make sure to also update 'zoom.gedit-plugin'" ; \
		fi \
	else \
...
mrproper: clean
	...
	find zoom/locale -name "*.mo" -exec rm {} \;
...
_create_localedir:
	if [ -n "$(LOCALE)" ] ; then \
		mkdir -p zoom/locale/$(LOCALE)/LC_MESSAGES ; \
	fi
